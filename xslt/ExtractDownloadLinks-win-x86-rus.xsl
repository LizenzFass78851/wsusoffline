...
<xsl:template match="*">
  <xsl:choose>
    <xsl:when test="name()='FileLocation'">
      ...
      <xsl:if test="contains(@Url, 'http://') and contains(@Url, '/directx9') and contains(@Url, '-x86-') and contains(@Url, '-rus') and contains(@Url, '.exe')">
        <xsl:value-of select="@Url"/>
        <xsl:text>&#10;</xsl:text>
      </xsl:if>
      <xsl:if test="contains(@Url, 'http://') and contains(@Url, '/mdac281') and contains(@Url, '-x86-') and contains(@Url, '-rus') and contains(@Url, '.exe')">
        <xsl:value-of select="@Url"/>
        <xsl:text>&#10;</xsl:text>
      </xsl:if>
      <xsl:if test="contains(@Url, 'http://') and (contains(@Url, '/windowsmedia6') or contains(@Url, '/windowsmedia9') or contains(@Url, '/windowsmedia10') or contains(@Url, '/windowsmedia11')) and contains(@Url, '-x86-') and contains(@Url, '-rus') and contains(@Url, '.exe')">
        <xsl:value-of select="@Url"/>
        <xsl:text>&#10;</xsl:text>
      </xsl:if>
