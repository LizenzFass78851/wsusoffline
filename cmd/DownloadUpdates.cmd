...
set WSUSOFFLINE_VERSION=12.5 (b72r2)
...
if exist ..\client\wddefs\x64-glb\mpas-fe.exe del ..\client\wddefs\x64-glb\mpas-fe.exe
if exist ..\client\wddefs\x86-glb\mpas-fe.exe del ..\client\wddefs\x86-glb\mpas-fe.exe
...
if "%VERIFY_DL%"=="1" (
  ...
  if not exist ..\client\md\hashes-msedge.txt copy /y "%TEMP%\hashes-msedge.txt" ..\client\md
  call :Log "Info: Saved integrity database for Microsoft Edge (Chromium) installation files"
)
...
for /F "usebackq tokens=1,2 delims=," %%j in ("%TEMP%\DynamicDownloadLinks-msedge.txt") do (
  if "%%k" NEQ "" (
    if not exist "..\client\msedge\%%k" (
      ...
      call :Log "Info: Downloaded/validated ..\client\msedge\%%k"
    ) else (
      echo Info: Skipping download of ..\client\msedge\%%k
      call :Log "Info: Skipping download of ..\client\msedge\%%k"
    )
  )
)
